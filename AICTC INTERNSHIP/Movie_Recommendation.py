...
# indices = pd.Series(movies.index, index=movies['title']).drop_duplicates()  # Create a Series to map movie titles to their index number

# def get_recommendations(title, cosine_sim=cosine_sim):
#     idx = indices.get(title)  # Get the index of the movie that matches the title
#     if idx is None:
...
#     movie_indices = [i[0] for i in sim_scores]  # Get the indices of the top 10 most similar movies

#     return movies['title'].iloc[movie_indices]  # Return the titles of the top 10 most similar movies
...
#             st.write(f"{i+1}. {rec}")
#     except:
#         st.write("Movie not found. Please check spelling or try another movie.")
